...
AWS Lambda function for identifying and pseudonymising the patient cohort that will be used across all subsequent
processing layers.
...
- First 13 rows are metadata (skipped during processing)
- Row 14 is the header row
- Accompanied by SHA256 checksum files (in `sha256sum` format: `<hash>  <filename>`)
- Named consistently (e.g., `data.csv` → `data.sha256`)

### Data Validation

- NHS numbers are validated using Modulus 11 checksum algorithm
- Invalid NHS numbers (wrong length, format, or checksum) are excluded
- Duplicates within each file are removed
